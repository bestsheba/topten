...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function __construct()
    {
        ...
        // sslcommerz_api_endpoint

        $this->setStoreId($this->setting->sslcommerz_store_id);
        $this->setStorePassword($this->setting->sslcommerz_store_password);
    }
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if (!empty($merchant_trans_id) && !empty($merchant_trans_amount)) {
            ...
            $requested_url = ($this->setting->sslcommerz_api_endpoint . $this->config['apiUrl']['order_validate'] . "?val_id=" . $val_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json");
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        $this->setApiUrl($this->setting->sslcommerz_api_endpoint . $this->config['apiUrl']['make_payment']);
